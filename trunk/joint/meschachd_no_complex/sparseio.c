...
void    sp_foutput(char *fname,SPMAT *A)
{
	int     i, j_idx, m /* , n */;
	SPROW  *rows;
	row_elt *elts;
        ...
        fp2 = fopen(fname,"wt");
        if (fp2 == NULL) {fprintf(stderr,"Unable to open %s\n",fname); exit(0);}
	fprintf(fp2,"SparseMatrix: ");
	if ( A == SMNULL )
	{
		fprintf(fp2,"*** NULL ***\n");
	...
	for ( i = 0; i < m; i++ )
	{
		...
		if ( ! (elts=rows[i].elt) )
		{
			fprintf(fp2,"*** NULL element list ***\n");
			continue;
		}
		for ( j_idx = 0; j_idx < rows[i].len; j_idx++ )
		{
			fprintf(fp2,"%d:%-20.15g ",elts[j_idx].col,
							elts[j_idx].val);
