...
#pragma omp critical(forward_write_files)
	{
		...
		Write3DModelForX3D((DirName / modelfilename).string(),
				...
				Info.Model.GetZCellSizes(), ShiftDepth,
				Info.Model.GetConductivities(),
	...
	for (size_t j = 0; j < nmeas; ++j)
	{
		boost::array<ThreeDModelBase::t3DModelData::index, 3> StationIndex =
				Info.Model.FindAssociatedIndices(Info.Model.GetMeasPosX()[j],
						Info.Model.GetMeasPosY()[j],
						Info.Model.GetMeasPosZ()[j]);
...
//                Hy1Inter, Hy2Inter, Zxx, Zxy, Zyx, Zyy);
		...
		FieldsToImpedance(Ex1[offset], Ex2[offset], Ey1[offset], Ey2[offset],
				Hx1[offset], Hx2[offset], Hy1[offset], Hy2[offset], Zxx, Zxy,
				Zyx, Zyy);
		...
		+ Info.C[site_index + 1] * Zyx.real();
				result.DistImpedance(meas_index + 1) = Info.C[site_index]
						* Zxx.imag() + Info.C[site_index + 1] * Zyx.imag();
				result.DistImpedance(meas_index + 2) = Info.C[site_index]
						* Zxy.real() + Info.C[site_index + 1] * Zyy.real();
				result.DistImpedance(meas_index + 3) = Info.C[site_index]
						* Zxy.imag() + Info.C[site_index + 1] * Zyy.imag();
				result.DistImpedance(meas_index + 4) = Info.C[site_index + 3]
						* Zyx.real() + Info.C[site_index + 2] * Zxx.real();
				result.DistImpedance(meas_index + 5) = Info.C[site_index + 3]
						* Zyx.imag() + Info.C[site_index + 2] * Zxx.imag();
				result.DistImpedance(meas_index + 6) = Info.C[site_index + 3]
						* Zyy.real() + Info.C[site_index + 2] * Zxy.real();
				result.DistImpedance(meas_index + 7) = Info.C[site_index + 3]
						* Zyy.imag() + Info.C[site_index + 2] * Zxy.real();
		...
		jif3D::rvec LQDerivativeFreq(const ForwardInfo &Info,
				const jif3D::rvec &Misfit, const jif3D::rvec &RawImpedance)
		{
...
#pragma omp critical(gradient_readema)
			...
			std::vector<std::complex<double> > XPolMoments1(nmeas),
					XPolMoments2(nmeas), YPolMoments1(nmeas), YPolMoments2(
							nmeas), Zeros(nmeas);
...
#pragma omp critical(gradient_writemodel_edip)
			{
				...
				Write3DModelForX3D((EdipDirName / modelfilename).string(),
						Info.Model.GetXCellSizes(), Info.Model.GetYCellSizes(),
						Info.Model.GetZCellSizes(), SourceObserve,
						Info.Model.GetConductivities(),
						Info.Model.GetBackgroundConductivities(),
				...
				WriteSourceFile((EdipDirName / sourcebfilename).string(),
						...
						Zeros, Zeros, Info.Model.GetZCoordinates(),
						Info.Model.GetZCellSizes(), nmodx, nmody);
			}
			...
			CalcU(EdipName.string(), XPolMoments1, YPolMoments1, Ux1_el, Uy1_el,
					Uz1_el, SourceXIndex, SourceYIndex,
					Info.Model.GetMeasPosZ(), Info.Model.GetZCoordinates(),
					Info.Model.GetZCellSizes(), nmodx, nmody, nmodz);
			//calculate the second polarization
			CalcU(EdipName.string(), XPolMoments2, YPolMoments2, Ux2_el, Uy2_el,
					Uz2_el, SourceXIndex, SourceYIndex,
					Info.Model.GetMeasPosZ(), Info.Model.GetZCoordinates(),
					Info.Model.GetZCellSizes(), nmodx, nmody, nmodz);
...
#pragma omp critical(gradient_writemodel_mdip)
			...
			for (size_t j = 0; j < nmeas; ++j)
			{
				...
				std::complex<double> Zxx(RawImpedance(offset),
						RawImpedance(offset + 1)), Zxy(RawImpedance(offset + 2),
						RawImpedance(offset + 3)), Zyx(RawImpedance(offset + 4),
						RawImpedance(offset + 5)), Zyy(RawImpedance(offset + 5),
						RawImpedance(offset + 6));
			}
			...
			for (size_t j = 0; j < nmod; ++j)
			{
				Volume = cell_sizex * cell_sizey
						* Info.Model.GetZCellSizes()[j % nmodz];
