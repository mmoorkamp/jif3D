...
#ifdef HAVEHPX
typedef hpx::serialization::output_archive oarchive;
typedef hpx::serialization::input_archive iarchive;
#else
    ...
    BOOST_AUTO_TEST_CASE (grid_struct_test)
      {
        ...
        std::generate(OutStruct.slow.begin(), OutStruct.slow.end(),
    ...
    BOOST_AUTO_TEST_CASE (data_struct_test)
      {
        ...
        std::generate(OutStruct.tcalc.begin(), OutStruct.tcalc.end(),
            jif3D::platform::drand48);
    ...
    BOOST_AUTO_TEST_CASE (rp_struct_test)
      {
        ...
        std::generate_n(std::back_inserter(OutStruct.len), nlen,
            jif3D::platform::drand48);
    ...
    BOOST_AUTO_TEST_CASE (ray_result_test)
      {
        ...
        std::generate_n(std::back_inserter(OutStruct.tcalc), ntcalc,
            jif3D::platform::drand48);
        ...
        for (size_t i = 0; i < nraypath; ++i)
          {
            ...
            for (size_t j = 0; j < InStruct.raypath[i].x.size(); ++j)
              {
                BOOST_CHECK_CLOSE(OutStruct.raypath[i].x[j], InStruct.raypath[i].x[j],
                    1e-4);
              }
            for (size_t j = 0; j < InStruct.raypath[i].y.size(); ++j)
              {
                BOOST_CHECK_CLOSE(OutStruct.raypath[i].y[j], InStruct.raypath[i].y[j],
                    1e-4);
              }
            for (size_t j = 0; j < InStruct.raypath[i].z.size(); ++j)
              {
                BOOST_CHECK_CLOSE(OutStruct.raypath[i].z[j], InStruct.raypath[i].z[j],
                    1e-4);
              }
            for (size_t j = 0; j < InStruct.raypath[i].len.size(); ++j)
              {
                BOOST_CHECK_CLOSE(OutStruct.raypath[i].len[j], InStruct.raypath[i].len[j],
                    1e-4);
              }
            for (size_t j = 0; j < InStruct.raypath[i].ele.size(); ++j)
              {
                BOOST_CHECK_EQUAL(OutStruct.raypath[i].ele[j],
                    InStruct.raypath[i].ele[j]);
              }
          }
