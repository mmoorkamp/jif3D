...
BOOST_AUTO_TEST_CASE (derivative_test)
  {
    ...
    auto exten =
        boost::extents[xcoords_m.size() - 1][ycoords_m.size() - 1][zcoords_m.size() - 1];
    ...
    for (size_t i = 0; i < TomoModel.GetData().num_elements(); ++i)
      {
        ...
        Velocity.origin()[i] = beta[i % (zsize - 1)] * (i + 1.0);
        Vp.origin()[i] = alpha[i % (zsize - 1)] * (i + 2.0);
        Density.origin()[i] = rho[i % (zsize - 1)] * (i + 5.0);
    ...
    jif3D::ThreeDModelObjective<jif3D::SurfaceWaveCalculator> TomoObjective(Calculator);
