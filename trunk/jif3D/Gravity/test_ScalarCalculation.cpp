...
BOOST_AUTO_TEST_CASE (box_gravity_calc_test)
  {
    ...
    double shifted = jif3D::CalcGravBoxTerm(10.0, 10.0, 0.0, 0.0, 0.0, 0.0, 20.0, 20.0,
        20.0);
...
BOOST_AUTO_TEST_CASE(model_gravity_boxcomp_test)
  {
    ...
    //again density is 1
    double boxtopofcube = jif3D::CalcGravBoxTerm(measx, measy, measz, 0.0, 0.0, 0.0, 20.0,
        20.0, 20.0);
    jif3D::GravimetryMatrix tensorbox = jif3D::CalcTensorBoxTerm(measx, measy, measz, 0.0,
    ...
    jif3D::ScalarGravityData SCData;
    jif3D::TensorGravityData TensorData;
    SCData.AddMeasurementPoint(measx, measy, measz);
    TensorData.AddMeasurementPoint(measx, measy, measz);
    ...
    jif3D::rvec gravmeas(ScalarCalculator->Calculate(GravityTest, SCData));
    jif3D::rvec tensmeas(TensorCalculator->Calculate(GravityTest, TensorData));
...
BOOST_AUTO_TEST_CASE(background_test)
  {
    ...
    Data.AddMeasurementPoint(rand() % 50000 + 2e4, rand() % 50000 + 2e4,
        0.0);
    ...
    jif3D::rvec gravmeas(ScalarCalculator->Calculate(GravityTest, Data));
    for (size_t i = 0; i < nmeas; ++i)
      {
...
BOOST_AUTO_TEST_CASE(model_gravity_1danaly_test)
  {
    ...
    jif3D::ThreeDModelBase::t3DModelDim XCD(nhorcells), YCD(nhorcells), ZCD(nzcells,
        500.0);
    ...
    for (size_t i = 0; i < nmeas; ++i)
      {
        SCData.AddMeasurementPoint(rand() % 50000 + 2e4, rand() % 50000 + 2e4,
            -1.0);
        TensorData.AddMeasurementPoint(rand() % 50000 + 2e4, rand() % 50000 + 2e4,
            -1.0);
      }
    std::vector<double> bg_dens, bg_thick;
    bg_dens += 1.0, 1.0, 5.0, 5.0;
    ...
    boost::shared_ptr<ScalarCalculatorType> ScalarCalculator(
        jif3D::CreateGravityCalculator<ScalarCalculatorType>::MakeScalar());
#ifdef HAVEGPU
    boost::shared_ptr<ScalarCalculatorType> CudaCalculator(jif3D::CreateGravityCalculator<CalculatorType>::MakeScalar(true));
    ...
    jif3D::rvec scalarmeas2(ScalarCalculator->Calculate(GravityTest, SCData));
    for (size_t i = 0; i < nmeas; ++i)
      {
...
BOOST_AUTO_TEST_CASE(scalar_caching_test)
  {
    ...
    boost::posix_time::ptime startfirst = boost::posix_time::microsec_clock::local_time();
    jif3D::rvec scalarmeas1(ScalarCalculator->Calculate(GravityTest, Data));
    boost::posix_time::ptime endfirst = boost::posix_time::microsec_clock::local_time();
    jif3D::rvec scalarmeas2(ScalarCalculator->Calculate(GravityTest, Data));
    boost::posix_time::ptime endsecond = boost::posix_time::microsec_clock::local_time();
...
BOOST_AUTO_TEST_CASE (lqderivative_test)
  {
    ...
    jif3D::rvec Deriv(ScalarCalculator->LQDerivative(GravityTest, Data, Misfit));
    ScalarCalculator->Calculate(GravityTest, Data);
    jif3D::rvec Compare(
        2.0
            * boost::numeric::ublas::prec_prod(
                ublas::trans(ScalarCalculator->GetSensitivities()), Misfit));
    //and test the caching, too
    jif3D::rvec Deriv2(ScalarCalculator->LQDerivative(GravityTest, Data, Misfit));
