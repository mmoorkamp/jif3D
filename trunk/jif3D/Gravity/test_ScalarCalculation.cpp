...
BOOST_AUTO_TEST_CASE  (box_gravity_calc_test)
    {
      ...
      BOOST_CHECK(fabs(jif3D::CalcGravBoxTerm(0.0, 0.0, 0.0, -10.0, -10.0, -10.0,
      ...
      double topofcube = jif3D::CalcGravBoxTerm(0.0, 0.0, -10.0, -10.0, -10.0,
          -10.0, 20.0, 20.0, 20.0);
      BOOST_CHECK_CLOSE(topofcube, 3.46426e-6, 0.1);
      double bottomofcube = jif3D::CalcGravBoxTerm(0.0, 0.0, 10.0, -10.0, -10.0,
          -10.0, 20.0, 20.0, 20.0);
      //check symmetry of results
      BOOST_CHECK_CLOSE(topofcube, -bottomofcube,
                std::numeric_limits<float>::epsilon());
      //check translation invariance
      double shifted = jif3D::CalcGravBoxTerm(10.0, 10.0, 0.0, 0.0, 0.0,
          0.0, 20.0, 20.0, 20.0);
      BOOST_CHECK_CLOSE(topofcube, shifted,
                      std::numeric_limits<float>::epsilon());

      double away1 = jif3D::CalcGravBoxTerm(1e5, 10.0, 10.0, -10.0, -10.0, -10.0,
          20.0, 20.0, 20.0);
      BOOST_CHECK(away1 < std::numeric_limits<double>::epsilon());
      double away2 = jif3D::CalcGravBoxTerm(100.0, 100.0, 10.0, -10.0, -10.0,
          -10.0, 20.0, 20.0, 20.0);
      BOOST_CHECK_CLOSE(away2, -1.873178e-9, 0.1);
      //check also for a very large cube
      BOOST_CHECK(fabs(jif3D::CalcGravBoxTerm(0.0, 0.0, 0.0, -1e6, -1e6, -1e6,
  ...
  BOOST_AUTO_TEST_CASE(model_gravity_boxcomp_test)
    {
      ...
      //again density is 1
      double boxtopofcube = jif3D::CalcGravBoxTerm(measx, measy, measz, 0.0, 0.0,
          0.0, 20.0, 20.0, 20.0);
      jif3D::GravimetryMatrix tensorbox = jif3D::CalcTensorBoxTerm(measx, measy,
      ...
      jif3D::rvec SensValues(boost::numeric::ublas::prec_prod(ScalarSensitivities,
              DensityVector));
  ...
  BOOST_AUTO_TEST_CASE(model_gravity_1danaly_test)
    {
      ...
      boost::shared_ptr<jif3D::MinMemGravMagCalculator> TensorCalculator(jif3D::CreateGravityCalculator<jif3D::MinMemGravMagCalculator>::MakeTensor());
      boost::shared_ptr<jif3D::FullSensitivityGravMagCalculator> ScalarCalculator(jif3D::CreateGravityCalculator<jif3D::FullSensitivityGravMagCalculator>::MakeScalar());
      boost::shared_ptr<jif3D::FullSensitivityGravMagCalculator> CudaCalculator(jif3D::CreateGravityCalculator<jif3D::FullSensitivityGravMagCalculator>::MakeScalar(true));
  ...
  BOOST_AUTO_TEST_CASE (lqderivative_test)
    {
      ...
      ScalarCalculator->Calculate(GravityTest);
      jif3D::rvec Compare(2.0 * boost::numeric::ublas::prec_prod(ublas::trans(ScalarCalculator->GetSensitivities()),Misfit));
