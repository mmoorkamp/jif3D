/*
 * ResistivityTransform.h
 *
 *  Created on: 19.11.2020
 *      Author: zhanjie and mm489
 */

#ifndef RESISTIVITYTRANSFORM_H_
#define RESISTIVITYTRANSFORM_H_
...
//! Transform generalized model parameters to resistivity
    /*! Similarly to DensityTransform, this class transforms
     * generalized model parameters to resistivity taking an
     ...
     * \f$ \rho = \1/exp( -a/s^2 - b/s -c), \f$
     * where s is slowness and the coefficient a,b,c can be specified
     * in the constructor.
     */
    class J3DEXPORT ResistivityTransform: public jif3D::GeneralModelTransform
      {
    private:
      ...
      const double c;
      //! Pointer to an object that translates generalized parameters to slowness, before we translate slowness to resistivity
    ...
    public:
      ...
      virtual ResistivityTransform* clone() const override
        {
          return new ResistivityTransform(*this);
        }
      //! Transform Generalized parameters in terms of slowness to resistivity using a functional relationship
      virtual jif3D::rvec GeneralizedToPhysical(const jif3D::rvec &FullModel) const override
        {
          ...
          jif3D::rvec Resistivity(FullModel.size());
          for (size_t i = 0; i < FullModel.size(); ++i)
            {
              if (RelModel.GetData().data()[i])
                {
                  Resistivity(i) = 1 / (std::exp(
                      -(a / (Slowness(i) * Slowness(i)) + b / Slowness(i) + c)));
                }
              else
                {
                  Resistivity(i) = replacevalue;
                }
            }
          return Resistivity;
        }
      //! Transform resistivity to Slowness and then Generalized Parameters
          ...
          for (size_t i = 0; i < nvals; ++i)
            {
              if (RelModel.GetData().data()[i])
                {
                  double vel = (-b + sqrt(b * b - 4.0 * a * (c + std::log(1 / FullModel(i)))))
                      / (2.0 * a);
                  Slowness(i) = 1.0 / vel;
                }
              else
       ...
       * then transforming to resistivity. In addition we can specify
       ...
       */
      ResistivityTransform(boost::shared_ptr<GeneralModelTransform> SlowTrans,
