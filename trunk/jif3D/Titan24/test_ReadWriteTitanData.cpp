...
BOOST_AUTO_TEST_CASE  (read_write_netcdf_test)
    {
      ...
      std::vector<int> ExIndices(nfreq*nstat),EyIndices(nfreq*nstat),HIndices(nfreq*nstat);
      ...
      jif3D::WriteTitanDataToNetCDF(filename,Frequencies,XCoord,YCoord,ZCoord,ExIndices,EyIndices,HIndices,Impedances,Error,C);
      ...
      jif3D::ReadTitanDataFromNetCDF(filename,ReadFrequencies,ReadXCoord,ReadYCoord,ReadZCoord,ReadExIndices,ReadEyIndices,ReadHIndices,ReadImpedances,ReadError,ReadC);
