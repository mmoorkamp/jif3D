...
int main()
{
	...
	jif3D::ReadImpedancesFromNetCDF(misfitfilename, Frequencies, StatX, StatY,
			StatZ, Misfit, Errors, C);
	...
	jif3D::ReadImpedancesFromNetCDF(impfilename, Frequencies, StatX, StatY,
			StatZ, Impedances, Errors, C);
	for (size_t i = 0; i < Indices.size(); ++i)
	{
		Errors(Indices.at(i)) = std::abs(Impedances(Indices.at(i)));
	}
	std::cout << "Modified: " << Indices.size() << " out of " << Impedances.size() << " data " << std::endl;
