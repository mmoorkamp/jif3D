...
int main()
{
    ...
    jif3D::WriteImpedancesToNetCDF(MeshFilename+"_data.nc", Model.GetFrequencies(),
        Model.GetMeasPosX(), Model.GetMeasPosY(), Model.GetMeasPosZ(), Impedances);
    jif3D::rvec Errors(Impedances.size(),0.0);
    jif3D::WriteImpedancesToMtt(MeshFilename+"_data.nc", Model.GetFrequencies(), Impedances, Errors);
