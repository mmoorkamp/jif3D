...
int main()
  {
    ...
    const double deltax = 200.0;
    const double deltay = 200.0;
    const int nx = 3 * (maxxmeas - minxmeas) / deltax;
    const int ny = 3 * (maxymeas - minymeas) / deltay;
    const int nz = 1;
    ...
    //    DataDiffVec, ModelWeight, DataError, 0.0, InvModel);
    ...
    for (size_t i = 0; i < nmeas; ++i)
      {
        Model.AddMeasurementPoint(PosX.at(i), PosY.at(i), newz);
      }
