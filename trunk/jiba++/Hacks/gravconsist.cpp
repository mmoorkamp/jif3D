...
int main() {
	...
	const int nrefine = 3;
	const double deltax = *std::max_element(XPosDiff.begin(), XPosDiff.end()) / nrefine;
	const double deltay = *std::max_element(YPosDiff.begin(), YPosDiff.end()) / nrefine;
	std::cout << "Delta X: " << deltax << " Delta Y: " << deltay << std::endl;
	const int nx = std::ceil(maxxmeas/deltax);
	const int ny = std::ceil(maxymeas/deltay);
	const int nz = 1;
	...
	//Model.SetOrigin(maxxmeas - minxmeas, maxymeas - minymeas, 0.0);
	...
	case jiba::ftg:
		jiba::Write3DTensorDataToVTK(modelfilename + ".inv_ftg.vtk",
				"grav_accel", InvData, Model.GetMeasPosX(),
				Model.GetMeasPosY(), Model.GetMeasPosZ());
