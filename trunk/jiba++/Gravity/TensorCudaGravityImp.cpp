    ...
    rvec TensorCudaGravityImp::CalcGridded(const size_t measindex,
        const ThreeDGravityModel &Model, rmat &Sensitivities)
      {
        ...
        result(1) = std::inner_product(currsens + ngrid, currsens + 2 * ngrid,
            Model.GetDensities().origin(), 0.0);
        result(3) = result(1);
        result(2) = std::inner_product(currsens + 2 * ngrid, currsens + 3
            * ngrid, Model.GetDensities().origin(), 0.0);
        result(6) = result(2);
        result(4) = std::inner_product(currsens + 3 * ngrid, currsens + 4
            * ngrid, Model.GetDensities().origin(), 0.0);
        result(5) = std::inner_product(currsens + 4 * ngrid, currsens + 5
            * ngrid, Model.GetDensities().origin(), 0.0);
        result(7) = result(5);
        result(8) = std::inner_product(currsens + 5 * ngrid, currsens + 6
            * ngrid, Model.GetDensities().origin(), 0.0);
        if (storesens)
          {
