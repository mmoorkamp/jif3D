...
namespace jiba
{
	...
	const std::map<X3DModel::ProblemType, std::string> Extension =
			boost::assign::map_list_of(X3DModel::MT, "MT")(X3DModel::EDIP,
					"EDIP")(X3DModel::MDIP, "MDIP");

	inline void CheckField(const std::vector<std::complex<double> > &Field,
			size_t nelem)
	{
		if (Field.size() != nelem)
			throw jiba::FatalException(
					"Number of read in elements in Field: "
							+ jiba::stringify(Field.size())
							+ " does not match expected: "
							+ jiba::stringify(nelem));
	}
...
/*		fs::directory_iterator end_itr; // default construction yields past-the-end
	...
	HPXMTCalculator::HPXMTCalculator(boost::filesystem::path TDir)
	{
		...
		{
			throw jiba::FatalException(
					"Cannot find .hnk files in current directory! ");
		}
	}
			...
			const jiba::ThreeDModelBase::t3DModelDim &ModelDepths)
	{
		...
		{
			while (mindex < ModelDepths.size()
					&& ModelDepths[mindex] < BGDepths[i])
			{
				++mindex;
			}
			if (mindex < ModelDepths.size()
					&& ModelDepths[mindex] != BGDepths[i])
			{
				throw jiba::FatalException(
						"Depth to background layer: "
								+ jiba::stringify(BGDepths[i])
			...
			size_t maxfreqindex)
	{
		...
		std::vector<double> BGDepths(Model.GetBackgroundThicknesses().size(),
				0.0);
		...
		FreqResult.reserve(nfreq-minfreqindex);
		CalculateFrequency_action temp;
		for (int i = minfreqindex; i < nfreq; ++i)
		{
			...
			try
			{
				...
				FreqResult.push_back(
						async(temp, locality_id, Model, i, TempDir.string()));
		...
		wait_all(FreqResult);
		std::cout << FreqResult.size() << std::endl;
		for (int i = minfreqindex; i < nfreq; ++i)
		{
			size_t startindex = nmeas * i * 8;
			std::cout << FreqResult[i].get().size() << std::endl;
			std::copy(FreqResult[i].get().begin(), FreqResult[i].get().end(),
	...
	{
		cmat result(2, 2);
		const std::complex<double> magdet = 1.
				/ (H(0, 0) * H(1, 1) - H(0, 1) * H(1, 0));
		result(0, 0) = magdet
				* (conj(A(0, 0)) * H(1, 1) - conj(A(0, 1)) * H(0, 1));
		result(0, 1) = magdet
				* (-conj(A(0, 0)) * H(1, 0) + conj(A(0, 1)) * H(0, 0));
		result(1, 0) = magdet
				* (conj(A(1, 0)) * H(1, 1) - conj(A(1, 1)) * H(0, 1));
		result(1, 1) = magdet
				* (-conj(A(1, 0)) * H(1, 0) + conj(A(1, 1)) * H(0, 0));
		return result;
	}
	...
	cmat MisfitToA(const rvec &Misfit, const size_t startindex)
	{
		...
		cmat result(2, 2);
		result(0, 0) = std::complex<double>(Misfit(startindex),
				Misfit(startindex + 1));
	...
	void CalcHext(const std::complex<double> &omega_mu,
			std::complex<double> &Xp1, std::complex<double> &Xp2,
			std::complex<double> &Yp1, std::complex<double> &Yp2,
	...
	void CalcU(const std::string &RootName,
			...
			std::vector<std::complex<double> > &Ux,
			std::vector<std::complex<double> > &Uy,
			std::vector<std::complex<double> > &Uz,
			const std::vector<size_t> &SourceXIndex,
			...
			const size_t ncellsx, const size_t ncellsy, const size_t ncellsz)
	{
		...
		{
			WriteSourceFile(DirName + sourceafilename, SourceXIndex,
					SourceYIndex, ObservationDepths, XPolMoments, YPolMoments,
					ZCellBoundaries, ZCellSizes, ncellsx, ncellsy);
		}
		RunX3D(RootName);
		{
	...
	rvec HPXMTCalculator::LQDerivativeFreq(const X3DModel &Model,
			const rvec &Misfit, size_t freqindex)
	{
		...
		for (size_t j = 0; j < nmeas; ++j)
		{
			ShiftDepth.push_back(
					Model.GetZCoordinates()[FindNearestCellBoundary(
							Model.GetMeasPosZ()[j], Model.GetZCoordinates(),
							Model.GetZCellSizes())]);
		}

		//read the fields from the forward calculation
		std::vector<std::complex<double> > Ex1_obs, Ex2_obs, Ey1_obs, Ey2_obs,
				Hx1_obs, Hx2_obs, Hy1_obs, Hy2_obs;
		{
			ReadEMO((ForwardDirName / emoAname).string(), Ex1_obs, Ey1_obs,
					Hx1_obs, Hy1_obs);
			ReadEMO((ForwardDirName / emoBname).string(), Ex2_obs, Ey2_obs,
					Hx2_obs, Hy2_obs);
		}
		...
		std::vector<std::complex<double> > Ex1_all, Ex2_all, Ey1_all, Ey2_all,
				Ez1_all, Ez2_all;
		...
		std::vector<std::complex<double> > XPolMoments1(nmeas), XPolMoments2(
				nmeas), YPolMoments1(nmeas), YPolMoments2(nmeas), Zeros(nmeas);
		...
		for (size_t j = 0; j < nmeas; ++j)
		{
			...
			cmat j_ext = CalcATimesH(
					MisfitToA(Misfit, siteindex),
		...
		//in parallel
		{
			...
			Write3DModelForX3D((EdipDirName / modelfilename).string(),
					...
					Model.GetZCellSizes(), ShiftDepth,
			...
			WriteSourceFile((EdipDirName / sourcebfilename).string(),
					SourceXIndex, SourceYIndex, Model.GetMeasPosZ(), Zeros,
					Zeros, Model.GetZCoordinates(), Model.GetZCellSizes(),
					nmodx, nmody);
		}
		...
		CalcU(EdipName.string(), XPolMoments1, YPolMoments1, Ux1_el, Uy1_el,
				Uz1_el, SourceXIndex, SourceYIndex, Model.GetMeasPosZ(),
				Model.GetZCoordinates(), Model.GetZCellSizes(), nmodx, nmody,
				nmodz);
		//calculate the second polarization
		CalcU(EdipName.string(), XPolMoments2, YPolMoments2, Ux2_el, Uy2_el,
				Uz2_el, SourceXIndex, SourceYIndex, Model.GetMeasPosZ(),
				Model.GetZCoordinates(), Model.GetZCellSizes(), nmodx, nmody,
				nmodz);
		...
		for (size_t j = 0; j < nmeas; ++j)
		{
			...
			FieldsToImpedance(Ex1_obs[offset], Ex2_obs[offset], Ey1_obs[offset],
					Ey2_obs[offset], Hx1_obs[offset], Hx2_obs[offset],
					Hy1_obs[offset], Hy2_obs[offset], Zxx, Zxy, Zyx, Zyy);
			CalcHext(omega_mu, XPolMoments1.at(j), XPolMoments2.at(j),
					YPolMoments1.at(j), YPolMoments2.at(j), Zxx, Zxy, Zyx, Zyy);
		}

		std::vector<std::complex<double> > Ux1_mag, Ux2_mag, Uy1_mag, Uy2_mag,
				Uz1_mag, Uz2_mag;
		//calculate the first polarization and read the adjoint fields
		CalcU(MdipName.string(), XPolMoments1, YPolMoments1, Ux1_mag, Uy1_mag,
				Uz1_mag, SourceXIndex, SourceYIndex, Model.GetMeasPosZ(),
				Model.GetZCoordinates(), Model.GetZCellSizes(), nmodx, nmody,
				nmodz);
		...
		CalcU(MdipName.string(), XPolMoments2, YPolMoments2, Ux2_mag, Uy2_mag,
				Uz2_mag, SourceXIndex, SourceYIndex, Model.GetMeasPosZ(),
				Model.GetZCoordinates(), Model.GetZCellSizes(), nmodx, nmody,
				nmodz);
	...
	rvec HPXMTCalculator::LQDerivative(const X3DModel &Model,
...
jiba::rvec CalculateFrequency(const jiba::X3DModel &Model, size_t freqindex,
		 std::string TempDirName)
{
	...
	fs::path RootName = TempDir
			/ jiba::MakeUniqueName(jiba::X3DModel::MT, freqindex);
	fs::path DirName = RootName.string() + jiba::dirext;
	std::vector<double> CurrFreq(1, Model.GetFrequencies()[freqindex]);
	std::vector<double> ShiftDepth;
	for (size_t j = 0; j < nmeas; ++j)
	{
		ShiftDepth.push_back(
				Model.GetZCoordinates()[jiba::FindNearestCellBoundary(
						Model.GetMeasPosZ()[j], Model.GetZCoordinates(),
						Model.GetZCellSizes())]);
	}
	...
	{
		jiba::MakeRunFile(RootName.string(), DirName.string());
		jiba::WriteProjectFile(DirName.string(), CurrFreq, jiba::X3DModel::MT,
				jiba::resultfilename, jiba::modelfilename);
		jiba::Write3DModelForX3D((DirName / jiba::modelfilename).string(),
				Model.GetXCellSizes(), Model.GetYCellSizes(),
				Model.GetZCellSizes(), ShiftDepth, Model.GetConductivities(),
				Model.GetBackgroundConductivities(),
				Model.GetBackgroundThicknesses());
	}
	//run x3d in parallel
	...
	for (size_t j = 0; j < nmeas; ++j)
	{
		...
		jiba::FieldsToImpedance(Ex1[offset], Ex2[offset], Ey1[offset],
				Ey2[offset], Hx1[offset], Hx2[offset], Hy1[offset], Hy2[offset],
				Zxx, Zxy, Zyx, Zyy);
