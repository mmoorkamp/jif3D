    ...
    for (unsigned int i = 0; i < nruns; ++i)
      {
        ...
        double deviation = (mybox-otherbox)/mybox;
        maxdifference = std::max(maxdifference,std::abs(deviation));
        differences.push_back(deviation);
      }

    std::ofstream outfile("diff.out");
    std::copy(differences.begin(),differences.end(),std::ostream_iterator<double>(outfile,"\n"));
    std::cout <<  "Maximum relative difference: " << maxdifference << std::endl;
