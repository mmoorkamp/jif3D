...
BOOST_AUTO_TEST_CASE(memory_test)
  {
    ...
    BOOST_CHECK(memory(NULL,0,1,"test") == NULL);
    // check simple allocation
    size_t length = 100;
    char * mem_test = memory(NULL,length,1,"test");
    BOOST_CHECK( mem_test != NULL);
    for (size_t i =0; i < length; ++i) mem_test[i] = i;
    // check reallocation
    mem_test = memory(mem_test,2*length,1,"test");
    ...
    BOOST_CHECK_THROW(memory(NULL,failsize,1,"throw"),std::runtime_error);
    BOOST_CHECK_THROW(memory(mem_test,failsize,1,"throw"),std::runtime_error);
  }
