    ...
    char *memory(char *prev_addr, size_t n, size_t size, std::string progname)
      {
        ...
        if (n == 0)
          {
            std::cerr << "\n\n!!!WARNING!!!: In " << progname
                << ", no memory was allocated, n = " << n << std::endl
                << std::endl;
        ...
        if (prev_addr) // if we have to reallocate
          {
            if ((tmp = (char *) realloc(prev_addr, n * size)) == NULL)
              {
                std::string ErrorString = "Fatal Error: " + progname
                    + " could not reallocate more memory, n = " + stringify(n);
        ...
        else //if we allocate new memory
          {
            if ((tmp = (char *) calloc(n, size)) == NULL)
              {
                std::string ErrorString = "Fatal Error: " + progname
                    + " could not allocate more memory, n = " + stringify(n);
                throw std::runtime_error(ErrorString);
              }
          }
        return (tmp);
