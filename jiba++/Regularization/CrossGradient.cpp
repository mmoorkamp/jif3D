...
void CrossGradient::ImplDataDifference(const jiba::rvec &Model, jiba::rvec &Diff)
          {
            ...
            FirstGradient.CalcMisfit(ublas::vector_range<const jiba::rvec>(Model,
                ublas::range(0, nmod / 2)));
            SecondGradient.CalcMisfit(ublas::vector_range<const jiba::rvec>(
                Model, ublas::range(nmod / 2, nmod)));
            ...
            ublas::vector_range<const jiba::rvec> SecondZGrad(
                SecondGradient.GetDataDifference(), ublas::range(nmod,
                    Diff.size()));
...
jiba::rvec CrossGradient::ImplGradient(const jiba::rvec &Model,
         const jiba::rvec &Diff)
       {
         ...
         FirstGradient.CalcGradient(ublas::vector_range<const jiba::rvec>(
             Model, ublas::range(0, halfmod)));
         SecondGradient.CalcGradient(ublas::vector_range<const jiba::rvec>(
             Model, ublas::range(halfmod, nmod)));
             ...
             for (size_t j = 0; j < ysize; ++j)
               {
                 for (size_t k = 0; k < zsize; ++k)
                   {
                     const size_t index = ModelGeometry.IndexToOffset(i, j, k);
                     const size_t frontindex = ModelGeometry.IndexToOffset(i
                         - 1, j, k);
                     Gradient(index) -= Diff(frontindex + halfmod)
                         * SecondGradient.GetDataDifference()(frontindex
                             + nmod);
                     Gradient(index + halfmod)
                         += Diff(frontindex + halfmod)
                             * FirstGradient.GetDataDifference()(frontindex
                                 + nmod);
             ...
             for (size_t j = 1; j < ysize; ++j)
               {
                 for (size_t k = 0; k < zsize; ++k)
                   {
                     const size_t index = ModelGeometry.IndexToOffset(i, j, k);
                     const size_t leftindex = ModelGeometry.IndexToOffset(i, j
                         - 1, k);
                     //for the x-component
                     Gradient(index)
                         += Diff(leftindex)
             ...
             for (size_t j = 0; j < ysize; ++j)
               {
                 for (size_t k = 1; k < zsize; ++k)
                   {
                     const size_t index = ModelGeometry.IndexToOffset(i, j, k);
                     const size_t topindex = ModelGeometry.IndexToOffset(i, j,
                         k - 1);
