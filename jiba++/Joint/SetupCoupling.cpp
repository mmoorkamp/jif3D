...
namespace jiba
{
	...
	SetupCoupling::SetupCoupling() :
			mincond(0.0), maxcond(0.0), minslow(0.0), maxslow(0.0), mindens(
					0.0), maxdens(0.0), density_a(0.0), density_b(0.0), cond_a(
					0.0), cond_b(0.0), cond_c(0.0)
	{
	}
	...
	po::options_description SetupCoupling::SetupOptions()
	{
		...
		desc.add_options()("crossgrad", "Use cross-gradient coupling")(
				"saltrel", "Use a parameter constraint designed for salt")(
				...
				"The quadratic term of the velocity-conductivity relationship")(
			...
			bool Wavelet)
	{
		...
		if (Wavelet)
		{
			WaveletTrans = boost::shared_ptr<jiba::GeneralModelTransform>(
					new jiba::WaveletModelTransform(
							StartModel.GetData().shape()[0],
		...
		if (vm.count("crossgrad") || vm.count("saltrel"))
		{
			...
			TomoTransform = boost::shared_ptr<jiba::GeneralModelTransform>(
					new jiba::MultiSectionTransform(3 * ngrid, 0, ngrid,
							SlownessTransform));
			...
			ConductivityTransform->AppendTransform(
					boost::shared_ptr<jiba::GeneralModelTransform>(
							new jiba::TanhTransform(std::log(mincond),
									std::log(maxcond))));
			ConductivityTransform->AppendTransform(
					boost::shared_ptr<jiba::GeneralModelTransform>(
			...
			MTTransform = boost::shared_ptr<jiba::GeneralModelTransform>(
					new jiba::MultiSectionTransform(3 * ngrid, 2 * ngrid,
							3 * ngrid, ConductivityTransform));
		}
		else
		{
			...
			bool substart)
	{
		...
		if (SeisMod.GetNModelElements() == ngrid)
		{
			std::copy(SeisMod.GetSlownesses().origin(),
					SeisMod.GetSlownesses().origin() + ngrid,
					SeisModel.begin());
			std::cout << "Transforming slowness model. " << std::endl;
			ublas::subrange(InvModel, 0, ngrid) =
					ublas::subrange(SlowTrans->PhysicalToGeneralized(SeisModel), 0, ngrid);
		}
		...
		if (GravMod.GetNModelElements() == ngrid)
		{
			...
			ublas::subrange(InvModel, ngrid, 2 * ngrid) =
					ublas::subrange(DensTrans->PhysicalToGeneralized(GravModel), ngrid, 2 * ngrid);
		}
		...
		if (MTMod.GetNModelElements() == ngrid)
		{
			...
			std::copy(MTMod.GetConductivities().origin(),
					MTMod.GetConductivities().origin() + ngrid,
					MTModel.begin());
			...
			ublas::subrange(InvModel, 2 * ngrid, 3 * ngrid) =
					ublas::subrange(CondTrans->PhysicalToGeneralized(MTModel), 2 * ngrid, 3 * ngrid);
		}
			...
			bool substart)
	{
		...
		{
			std::fill_n(ublas::subrange(InvModel, ngrid, 2 * ngrid).begin(),
					ngrid, 0.0);
		}
		...
		{
			std::fill_n(ublas::subrange(InvModel, 2 * ngrid, 3 * ngrid).begin(),
					ngrid, 0.0);
		}
		...
		{
			Objective.AddObjective(SaltRel, SaltRelTrans, saltrellambda,
					"SaltRel");
		}
		//finally we construct the regularization terms
		//we ask for a weight and construct a regularization object
			...
			bool substart)
	{
		...
		{
			jiba::rvec TomoCovar(3 * ngrid);
			SetupModelCovar(TomoCovar, InvModel, Regularization->GetDataError(),
					ngrid);
			...
			bool substart)
	{
		...
		{
			SetupCrossGradModel(InvModel, ModelGeometry, SeisMod, GravMod,
		...
		else
		{
			...
			{
				SetupFixedCouplingModel(InvModel, ModelGeometry, SeisMod,
						GravMod, MTMod, Objective, Regularization, substart);
			}
		}
	}
}
