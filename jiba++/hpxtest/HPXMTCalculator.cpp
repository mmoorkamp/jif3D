...
namespace jiba
  {
        ...
        for (int i = minfreqindex; i < maxindex; ++i)
          {
            ...
            FreqResult.push_back(async(temp, locality_id, Model, i, TempDir.string()));
    ...
    rvec HPXMTCalculator::LQDerivativeFreq(const X3DModel &Model, const rvec &Misfit,
        size_t freqindex)
      {
    ...
    rvec HPXMTCalculator::LQDerivative(const X3DModel &Model, const rvec &Misfit,
...
jiba::rvec CalculateFrequency(const jiba::X3DModel &Model, size_t freqindex,
    std::string TempDirName)
  {
    ...
    fs::path RootName = TempDir / jiba::MakeUniqueName(jiba::X3DModel::MT, freqindex);
