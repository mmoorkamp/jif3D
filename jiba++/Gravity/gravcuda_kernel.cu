...
__global__ void CalcScalarMeas(const double x_meas, const double y_meas,
		...
		const double *ZSizes, const int nx,
		const int ny, const int nz, double *returnvalue) {
