		...
		RP_STRUCT *raypath)
{
	...
	jif3D::PodvinTime3D().time_3d(&SlowBuffer[0], &tt[0], nx3, ny3, nz3, Xs, Ys,
			Zs, delta_num, 0);
...
#pragma omp for
		...
		{
			ForwardModShot(i, uniqueshots, geo, grid, data, raypath);
		}
	...
	for (size_t i = 0; i < data->ndata_seis; i++)
	{
		if (data->tcalc[i] == -1.0)
		{
			...
			throw jif3D::FatalException(
					"For the shot-receiver combination"
							+ jif3D::stringify(i + 1)
		...
		float *data)
{
	...
	if (!ok)
	{
		std::string error = "Interpolation point is out of the grid! x: "
				...
				+ " nx2: " + stringify(nx2) + " ny2: " + stringify(ny2)
				+ " nz2: " + stringify(nz2) + "\n";
		...
		float *Xr, float *Yr, float *Zr, int nrec, RP_STRUCT *rp)
{
	...
	for (i = 0; i < nrec; i++)
	{
		...
		next_cell = RayBackTrace(gradient(0), gradient(1), gradient(2), cell,
				tt, ny, nz);
		...
		rp[i].len[0] =
				sqrt(
						(next_cell.xpos + next_cell.xno - cell.xpos - cell.xno)
								* (next_cell.xpos + next_cell.xno - cell.xpos
										- cell.xno)
								+ (next_cell.ypos + next_cell.yno - cell.ypos
										- cell.yno)
										* (next_cell.ypos + next_cell.yno
												- cell.ypos - cell.yno)
								+ (next_cell.zpos + next_cell.zno - cell.zpos
										- cell.zno)
										* (next_cell.zpos + next_cell.zno
		...
		if (next_cell.xno == 0 || next_cell.xno == nx1 || next_cell.yno == 0
				|| next_cell.yno == ny1 || next_cell.zno == 0
				|| next_cell.zno == nz1)
		{
			//printf(
...
jif3D::rvec TimeGrad(int x, int y, int z, float *tt, int ny, int nz)
{
	...
	grad[0] = (-Traveltimes(x + 1, y, z) - Traveltimes(x + 1, y, z + 1)
			...
			+ Traveltimes(x, y, z) + Traveltimes(x, y, z + 1)
			+ Traveltimes(x, y + 1, z) + Traveltimes(x, y + 1, z + 1)) / 4; /*x-component*/

	grad[1] = (-Traveltimes(x, y + 1, z) - Traveltimes(x, y + 1, z + 1)
			- Traveltimes(x + 1, y + 1, z) - Traveltimes(x + 1, y + 1, z + 1)
			+ Traveltimes(x, y, z) + Traveltimes(x, y, z + 1)
			+ Traveltimes(x + 1, y, z) + Traveltimes(x + 1, y, z + 1)) / 4; /*y-component*/

	grad[2] = (-Traveltimes(x, y, z + 1) - Traveltimes(x, y + 1, z + 1)
			- Traveltimes(x + 1, y, z + 1) - Traveltimes(x + 1, y + 1, z + 1)
			+ Traveltimes(x, y, z) + Traveltimes(x, y + 1, z)
			+ Traveltimes(x + 1, y, z) + Traveltimes(x + 1, y + 1, z)) / 4; /*z-component*/
		...
		CELL_STRUCT cell, float *tt, int ny, int nz)
{
			...
			|| (cell.dirx_i == -1 && gradx >= 0))
	{
		if (gradx > 0)
		{
			...
			if (1 >= tmp_ypos && tmp_ypos >= 0 && 1 >= tmp_zpos
					&& tmp_zpos >= 0)
			{
				next_cell.xpos = 0;
			...
			|| (cell.diry_i == -1 && grady >= 0))
	{
		if (grady > 0)
		{
			...
			if (1 >= tmp_xpos && tmp_xpos >= 0 && 1 >= tmp_zpos
					&& tmp_zpos >= 0)
			{
			...
			|| (cell.dirz_i == -1 && gradz >= 0))
	{
		if (gradz > 0)
		{
			...
			if (1 >= tmp_xpos && tmp_xpos >= 0 && 1 >= tmp_ypos
					&& tmp_ypos >= 0)
			{
				next_cell.xpos = tmp_xpos;
				next_cell.ypos = tmp_ypos;
		...
		if (grady < 0)
		{
			...
			if (1 >= tmp_zpos && tmp_zpos >= 0)
			{
				...
				gradient1 = TimeGrad(cell.xno + cell.dirx_i, cell.yno - 1,
						cell.zno, tt, ny, nz);
				gradient2 = TimeGrad(cell.xno, cell.yno - 1, cell.zno, tt, ny,
						nz);
		...
		if (gradz < 0)
		{
			...
			if (1 >= tmp_ypos && tmp_ypos >= 0)
			{
				...
				gradient1 = TimeGrad(cell.xno + cell.dirx_i, cell.yno,
						cell.zno - 1, tt, ny, nz);
				gradient2 = TimeGrad(cell.xno, cell.yno, cell.zno - 1, tt, ny,
						nz);
				if (gradient1[0] + gradient2[0] >= 0)
					diff = cell.dirx_i;
				else
		...
		if (gradx < 0)
		{
			...
			if (1 >= tmp_zpos && tmp_zpos >= 0)
			{
				...
				gradient1 = TimeGrad(cell.xno - 1, cell.yno + cell.diry_i,
						cell.zno, tt, ny, nz);
				gradient2 = TimeGrad(cell.xno - 1, cell.yno, cell.zno, tt, ny,
						nz);
	...
	if (((cell.dirz_i == 1 && gradz > 0) || (cell.dirz_i == -1 && gradz < 0)))
	{
		if (gradx < 0)
		{
			...
			if (1 >= tmp_ypos && tmp_ypos >= 0)
			{
				/*Comparison of the gradients(in the neighboring cells) to determine the next cell:*/
				/*Calculate the traveltime gradient*/
				gradient1 = TimeGrad(cell.xno - 1, cell.yno,
						cell.zno + cell.dirz_i, tt, ny, nz);
				gradient2 = TimeGrad(cell.xno - 1, cell.yno, cell.zno, tt, ny,
						nz);
		...
		if (grady < 0)
		{
			...
			if (1 >= tmp_xpos && tmp_xpos >= 0)
			{
				/*Comparison of the gradients(in the neighboring cells) to determine the next cell:*/
				/*Calculate the traveltime gradient*/
				gradient1 = TimeGrad(cell.xno, cell.yno - 1,
						cell.zno + cell.dirz_i, tt, ny, nz);
				gradient2 = TimeGrad(cell.xno, cell.yno - 1, cell.zno, tt, ny,
						nz);
				if (gradient1[2] + gradient2[2] >= 0)
