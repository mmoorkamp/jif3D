...
StochasticCovariance::StochasticCovariance(size_t x, size_t y, size_t z,
		double ma, double mnu, double msigma) :
		nx(x), ny(y), nz(z), a(ma), nu(mnu), sigma(msigma), HaveInv(false) {
	distindex = 0;
	double factor = sigma * sigma
			/ (std::pow(2, nu - 1) * boost::math::tgamma(nu));
	...
	for (double currval = 0; currval <= max; currval += delta) {
		double res =
				(currval == 0.0) ?
						1.0 :
						factor * std::pow(currval/a, nu)
								* boost::math::cyl_bessel_k(nu, currval/a);
