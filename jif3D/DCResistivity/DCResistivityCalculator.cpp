...
namespace jif3D
  {
    ...
    void DCResistivityCalculator::ModelToStruct(const ThreeDDCResistivityModel &Model,
        const DCResistivityData &Data, jif3D::GEOMETRY_RES &geo,
        jif3D::GRID_STRUCT_RES &grid)
      {
        ...
        if (Data.GetSourcePosPosZ()[0] == 0.0)
          {
            std::transform(Data.GetSourcePosPosZ().begin(), Data.GetSourcePosPosZ().end(),
                geo.PosSz.begin(), [zshift](double pos)
                  { return pos+zshift;});
            std::transform(Data.GetSourceNegPosZ().begin(), Data.GetSourceNegPosZ().end(),
