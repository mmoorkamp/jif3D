...
namespace jif3D
  {
    ...
    void DCResistivityCalculator::Allocate(const size_t ngrid,
        const size_t nshot, const size_t nmeaspoint, const size_t ndata)
      {
    ...
    rvec DCResistivityCalculator::Calculate(const ModelType &Model)
      {
        ...
        {
            std::transform(Model.GetSourcePosPosZ().begin(), Model.GetSourcePosPosZ().end(),
                geo.PosSz.begin(), [](double pos)
                  { return pos;});
            std::transform(Model.GetSourceNegPosZ().begin(), Model.GetSourceNegPosZ().end(),
                geo.NegSz.begin(), [](double pos)
                  { return pos;});
            std::transform(Model.GetMeasPosZ().begin(), Model.GetMeasPosZ().end(),
                geo.rz1.begin(), [](double pos)
                  { return pos;});
            std::transform(Model.GetMeasSecPosZ().begin(), Model.GetMeasSecPosZ().end(),
                geo.rz2.begin(), [](double pos)
                  { return pos;});
        }
        geo.nsource = nshot;
    ...
    rvec DCResistivityCalculator::LQDerivative(const ModelType &Model, const rvec &Misfit)
      {
        ...
        //fill the gradient structure
        grad.wdwmisfit.resize(ndata);
        grad.gradient.resize(ngrid);
        for (size_t i=0; i<ndata; i++)
        {
        	grad.wdwmisfit[i]=Misfit[i];
        }
        //now we can calculate gradient of objective function
        ResGradient(geo, grid, &grad);

        //return the result as a vector
        jif3D::rvec result(ngrid, 0.0);
        for (int ng=0; ng<ngrid; ng++)
        {
        	result[ng]=grad.gradient[ng];
        }
