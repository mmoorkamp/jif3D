...
namespace jif3D
  {
    ...
    jif3D::rvec ResForward(const GEOMETRY_RES &geo, const GRID_STRUCT_RES &grid,
        size_t ndata_res)
      {
        ...
        for (size_t i = 0; i < naz; i++)
          {
            SV_BC.push_back(
                ST_BC(lzs_BC[i] + nax + nay, jzs_BC[i] + nax + nay, 1.0 / kzs_BC[i]));
          }
        ...
        for (size_t i = 0; i < geo.nsource; i++)
          {
            std::vector<double> pvel(np, 0), nvel(np, 0), pveimagl(np, 0), nveimagl(np,
                0), U_H(np, 0), q_corr(np, 0), Q1(np, 0), Q2(np, 0), Q;
            Eigen::VectorXd permutationeq(np), permutationeu, eu(np), eforwarddata, eU_H(
                np), eq_corr(np);
                ...
                for (size_t n = 0; n < grid.nx; n++)
                  {
                    ...
                    pvel[n + m * grid.nx + l * grid.nx * grid.ny] = std::sqrt(
                        (cellcenterxpos[n] - geo.PosSx[i])
                            * (cellcenterxpos[n] - geo.PosSx[i])
                            + (cellcenterypos[m] - geo.PosSy[i])
                                * (cellcenterypos[m] - geo.PosSy[i])
                            + (cellcenterzpos[l] - geo.PosSz[i])
                                * (cellcenterzpos[l] - geo.PosSz[i]));
                    nvel[n + m * grid.nx + l * grid.nx * grid.ny] = std::sqrt(
                        (cellcenterxpos[n] - geo.NegSx[i])
                            * (cellcenterxpos[n] - geo.NegSx[i])
                            + (cellcenterypos[m] - geo.NegSy[i])
                                * (cellcenterypos[m] - geo.NegSy[i])
                            + (cellcenterzpos[l] - geo.NegSz[i])
                                * (cellcenterzpos[l] - geo.NegSz[i]));
                    //norm of imaginary current electrode and 1st potential electrode
                    pveimagl[n + m * grid.nx + l * grid.nx * grid.ny] = std::sqrt(
                        (cellcenterxpos[n] - geo.PosSx[i])
                            * (cellcenterxpos[n] - geo.PosSx[i])
                            + (cellcenterypos[m] - geo.PosSy[i])
                                * (cellcenterypos[m] - geo.PosSy[i])
                            + (cellcenterzpos[l] + geo.PosSz[i])
                                * (cellcenterzpos[l] + geo.PosSz[i]));
                    nveimagl[n + m * grid.nx + l * grid.nx * grid.ny] = std::sqrt(
                        (cellcenterxpos[n] - geo.NegSx[i])
                            * (cellcenterxpos[n] - geo.NegSx[i])
                            + (cellcenterypos[m] - geo.NegSy[i])
                                * (cellcenterypos[m] - geo.NegSy[i])
                    ...
                    U_H[n + m * grid.nx + l * grid.nx * grid.ny] = (1.0
                        / (grid.avg_cond * 4.0 * 3.1415926))
                        * (1.0 / pvel[n + m * grid.nx + l * grid.nx * grid.ny]
                            - 1.0 / nvel[n + m * grid.nx + l * grid.nx * grid.ny]
                            + 1.0 / pveimagl[n + m * grid.nx + l * grid.nx * grid.ny]
                            - 1.0 / nveimagl[n + m * grid.nx + l * grid.nx * grid.ny]);
                  }
                ...
                if (U_H[Uid] == singularityvalue || U_H[Uid] == -singularityvalue)
                  {
                    ...
                    idz = (idz - idy) / grid.ny;
                    if (idz == 0)
                      U_H[Uid] = (U_H[idx + 1 + idy * grid.nx + idz * grid.nx * grid.ny]
                          + U_H[idx + (idy + 1) * grid.nx + idz * grid.nx * grid.ny]
                          + U_H[idx + idy * grid.nx + (idz + 1) * grid.nx * grid.ny]
                          + U_H[idx - 1 + idy * grid.nx + idz * grid.nx * grid.ny]
                          + U_H[idx + (idy - 1) * grid.nx + idz * grid.nx * grid.ny])
                          / 5.0;
                    else
                      U_H[Uid] = (U_H[idx + 1 + idy * grid.nx + idz * grid.nx * grid.ny]
                ...
                if (i == geo.sno[Qi])
                  {
                    ...
                    double R1S1 = std::sqrt(
                        (geo.rx1[Qi] - geo.PosSx[i]) * (geo.rx1[Qi] - geo.PosSx[i])
                            + (geo.ry1[Qi] - geo.PosSy[i])
                                * (geo.ry1[Qi] - geo.PosSy[i]));
                    double R2S1 = std::sqrt(
                        (geo.rx2[Qi] - geo.PosSx[i]) * (geo.rx2[Qi] - geo.PosSx[i])
                            + (geo.ry2[Qi] - geo.PosSy[i])
                                * (geo.ry2[Qi] - geo.PosSy[i]));
                    double R1S2 = std::sqrt(
                        (geo.rx1[Qi] - geo.NegSx[i]) * (geo.rx1[Qi] - geo.NegSx[i])
                            + (geo.ry1[Qi] - geo.NegSy[i])
                                * (geo.ry1[Qi] - geo.NegSy[i]));
                    double R2S2 = std::sqrt(
                        (geo.rx2[Qi] - geo.NegSx[i]) * (geo.rx2[Qi] - geo.NegSx[i])
                            + (geo.ry2[Qi] - geo.NegSy[i])
                                * (geo.ry2[Qi] - geo.NegSy[i]));
                    GeometryFactor.push_back(
                        2 * 3.1415926 / (1 / R1S1 - 1 / R2S1 + 1 / R2S2 - 1 / R1S2));
            ...
            for (size_t ndatatemp = 0; ndatatemp < count; ndatatemp++)
              {
                if (GeometryFactor[ndatatemp] == singularityvalue
                    || GeometryFactor[ndatatemp] == -singularityvalue)
                  {
                  ...
                  {
                    datatemp.push_back(
                        eforwarddata(ndatatemp) * GeometryFactor[ndatatemp]);
    ...
    jif3D::rvec ResGradient(const GEOMETRY_RES &geo, const GRID_STRUCT_RES &grid,
        const jif3D::rvec &wdwmisfit)
      {
         ...
         */
        std::vector<double> pvel(np, 0), nvel(np, 0), pveimagl(np, 0), nveimagl(np, 0),
            U_H(np, 0), q_corr(np, 0), Q1(np, 0), Q2(np, 0), Q;
                ...
                if (i == geo.sno[Qi])
                  {
                    ...
                    double R2S2 = std::sqrt(
                        (geo.rx2[Qi] - geo.NegSx[i]) * (geo.rx2[Qi] - geo.NegSx[i])
                            + (geo.ry2[Qi] - geo.NegSy[i])
                                * (geo.ry2[Qi] - geo.NegSy[i]));
                    GeometryFactor = 2 * 3.1415926
                        / (1 / R1S1 - 1 / R2S1 + 1 / R2S2 - 1 / R1S2);

                    if (GeometryFactor == singularityvalue
                        || GeometryFactor == -singularityvalue)
                      {
