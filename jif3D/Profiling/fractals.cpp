...
int hpx_main()
  {
    ...
    std::cout << InValues.size() << " Values initialized in " << t.elapsed() << "s. \n";

    t.restart();
    hpx::transform(hpx::execution::parallel_policy(), InValues.begin(), InValues.end(),
        OutValues.begin(), [max_iteration](my_complex P) -> int
