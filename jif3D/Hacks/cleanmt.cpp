        ...
        for (size_t k = 2; k < 6; ++k)
          {
            ...
            std::cout << "Frequency: " << Freq.at(i) << " " << minthresh << " " << median
                << " " << maxthresh << std::endl;
            std::string filename = "Curr_" + jif3D::stringify(Freq.at(i)) + "_"
                + jif3D::stringify(k) + ".out";
...
//			std::cin >> maxselect;
            ...
            size_t currindex = 0;
            double minmaxerr = 2 * std::abs(median);
            for (size_t j = i * nstat * 8; j < (i + 1) * nstat * 8; j += 8)
              {
                ...
                if (Imp(j + k) < minthresh || Imp(j + k) > maxthresh)
                  {
                    ...
                    Err(j + k) = std::max(minmaxerr, Err(j + k));
                  }
                CurrErr(currindex) = Err(j + k);
                currindex++;
              }
            std::string impfilename = "Imp_" + jif3D::stringify(Freq.at(i)) + "_"
                + jif3D::stringify(k) + ".vtk";
            std::string errfilename = "Err_" + jif3D::stringify(Freq.at(i)) + "_"
                + jif3D::stringify(k) + ".vtk";
        ...
        for (size_t j = i * nstat * 8; j < (i + 1) * nstat * 8; j += 8)
          {
            Err(j + 2) = sqrt(Err(j + 2) * Err(j + 2) + Err(j + 3) * Err(j + 3));
            Err(j + 3) = Err(j + 2);
            Err(j + 4) = sqrt(Err(j + 4) * Err(j + 4) + Err(j + 5) * Err(j + 5));
            Err(j + 5) = Err(j + 4);
          }
        std::cout << std::endl;
      }
