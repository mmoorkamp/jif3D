...
int main()
  {
    ...
    size_t iteration = 0;
    const size_t ndata = TomoData.GetData().size();
    do
      {
        ...
        try
          {
            ...
            jif3D::rvec TomoModel = ModelTransform->GeneralizedToPhysical(InvModel);
            std::copy(TomoModel.begin(), TomoModel.end(), Model.SetSlownesses().origin());
            Model.WriteVTK(modelfilename + jif3D::stringify(iteration) + ".tomo.inv.vtk");
    ...
    jif3D::rvec InvData(jif3D::TomographyCalculator().Calculate(Model,TomoData));
    TomoData.SetDataAndErrors(std::vector<double>(InvData.begin(),InvData.end()),TomoData.GetErrors());
