...
BOOST_AUTO_TEST_CASE(ftginvar_sens_test)
  {
    ...
    boost::shared_ptr<CalculatorType> InvCalculator = boost::shared_ptr<CalculatorType>(
    ...
    MakeRandomModel(GravityTest, Data, ncells, nmeas, true);

    jif3D::rvec InvDirect(InvCalculator->Calculate(GravityTest, Data));
    jif3D::rvec InvCached(InvCalculator->Calculate(GravityTest, Data));
...
BOOST_AUTO_TEST_CASE(ftginvar_deriv_test)
  {
    typedef typename jif3D::FullSensitivityGravMagCalculator<jif3D::TensorGravityData> CalculatorType;
    boost::shared_ptr<CalculatorType> CacheCalculator = boost::shared_ptr<CalculatorType>(
        jif3D::CreateGravityCalculator<CalculatorType>::MakeTensor());

    typedef typename jif3D::FullSensitivityGravMagCalculator<jif3D::TensorGravityData> CompType;
    ...
    jif3D::rvec InvDirect(CompCalculator->LQDerivative(GravityTest, Data, Misfit));
    jif3D::rvec InvCached1(CacheCalculator->LQDerivative(GravityTest, Data, Misfit));
    jif3D::rvec InvCached2(CacheCalculator->LQDerivative(GravityTest, Data, Misfit));
...
BOOST_AUTO_TEST_CASE(diff_calc_test)
  {
    ...
    jif3D::rvec RawData(RawCalculator->Calculate(GravityTest, Data));
    const jif3D::rmat RawSens(RawCalculator->GetSensitivities());

    jif3D::rvec InvData(InvCalculator->Calculate(GravityTest, Data));
    ...
    jif3D::rvec Misfit(nmeas);
    std::fill(Misfit.begin(), Misfit.end(), 1.0);
    jif3D::rvec InvDeriv(InvCalculator->LQDerivative(GravityTest, Data, Misfit));
